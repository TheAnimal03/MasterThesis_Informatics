...
class Yolo_loss(nn.Module):
    def __init__(self, n_classes=80, n_anchors=3, device=None, batch=2):
        ...
        out_ch = n_anchors * (5+n_classes)
        self.adapter_convs = [None]*3
        self.adapter_convs[0] = nn.Conv2d(32, out_ch, 1)
        self.adapter_convs[1] = nn.Conv2d(96, out_ch, 1)
        self.adapter_convs[2] = nn.Conv2d(1280, out_ch, 1)
    ...
    def forward(self, xin, labels=None):
        ...
        for output_id, output in enumerate(xin):
            ...
            output = self.adapter_convs[output_id](output)
...
def train(model, device, config, anchors_mask, num_classes, epochs=5, batch_size=1, save_cp=True, resume=False, log_step=20, img_scale=0.5):
    ...
    for epoch in range(epoch, epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', ncols=50) as pbar:
            ...
            statstitle = ['Date', 'Results of epoch: ', 'AP: ', 'AP50: ', 'AP75: ', 'AP_small: ', 'AP_medium: ', 'AP_large: ', 'AR1: ', 'AR10: ', 'AR100: ', 'AR_small: ', 'AR_medium: ', 'AR_large: ']
            statsval = [savetime, epoch, stats[0], stats[1], stats[2], stats[3], stats[4], stats[5], stats[6], stats[7], stats[8], stats[9], stats[10], stats[11]]
            ...
            if save_cp:
                ...
                #save_checkpoint(save_cp, epoch, model, optim.lr_scheduler, optimizer)
                #save_checkpoint(config.checkpoints, epoch, loss.item(), model,lr_scheduler, optimizer)
#
...
#               self.anchor_w.append(anchor_w)
#               self.anchor_h.append(anchor_h)
#               print('yololoss: ', grid_x.size())
#         # te = grid_x[0]
#         # print('yololoss: ', te.size())
...
#     def forward(self, xin, labels=None):
#         loss, loss_xy, loss_wh, loss_obj, loss_cls, loss_l2 = 0, 0, 0, 0, 0, 0
#         #print('Batchsize Test: ', xin[0].size())
#         #te = self.grid_x[1]
#         #print('test gridx', te.size())
#         output_id, output = enumerate(xin)

#         for output_id, output in enumerate(xin):
#            # print('Batchsize: ', output.shape[0])
#             # print('test gridx-----', output_id)
#             # print('test gridx-----', output.size())

      
#             batchsize = output.shape[0]
#             fsize = output.shape[2]
#             n_ch = -48 + self.n_classes 
#             #print('Test size: ', n_ch)
#             #print('Anchor: ', output.size())
#             # te = self.grid_x[output_id]
#             # print('test gridx', te.size())
...
#             pred = output[..., :4].clone()
#             check  = self.grid_x[output_id]
#             pred_ = pred[..., 0]
#             print('Anchor: ', check.size())
#             print('Anchor: ', pred_.size())
#             pred[..., 0] += self.grid_x[output_id]
#             pred[..., 1] += self.grid_y[output_id]
#             pred[..., 2] = torch.exp(pred[..., 2]) * self.anchor_w[output_id]
#             pred[..., 3] = torch.exp(pred[..., 3]) * self.anchor_h[output_id]
...
#     if cfg.use_mobilenetv2_cfg:
#         model_url = 'https://raw.githubusercontent.com/d-li14/mobilenetv2.pytorch/master/pretrained/mobilenetv2-c5e733a8.pth'
#         model = mobilenetv2(model_url)
#         #model = MobileNetV2.mobilenet_v2(True)
#         #print(model)
#     else:
